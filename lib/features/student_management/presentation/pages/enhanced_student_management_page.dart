...
import '../widgets/enhanced_student_detail_dialog.dart';
...
class EnhancedStudentManagementPage extends StatefulWidget {
  const EnhancedStudentManagementPage({super.key});

  @override
  State<EnhancedStudentManagementPage> createState() => _EnhancedStudentManagementPageState();
}

class _EnhancedStudentManagementPageState extends State<EnhancedStudentManagementPage>
    ...
    return AppBar(
      ...
      actions: [
        ...
        PopupMenuButton<StudentSortBy>(
          ...
          onSelected: (sortBy) {
            context.read<EnhancedStudentProvider>().setSortBy(sortBy);
          },
          itemBuilder: (context) => [
            const PopupMenuItem(value: StudentSortBy.name, child: Text('Nom')),
            const PopupMenuItem(value: StudentSortBy.matricule, child: Text('Matricule')),
            const PopupMenuItem(value: StudentSortBy.level, child: Text('Niveau')),
            const PopupMenuItem(value: StudentSortBy.status, child: Text('Statut')),
            const PopupMenuItem(value: StudentSortBy.gpa, child: Text('GPA')),
            const PopupMenuItem(value: StudentSortBy.createdAt, child: Text('Date de création')),
          ],
        ),
  ...
  void _handleQuickAction(String action, EnhancedStudentModel student) {
    ...
    switch (action) {
      ...
      case 'delete':
        ...
        break;
      case 'activate':
        provider.updateStudentStatus(student.id, StudentStatus.enrolled);
        break;
      case 'deactivate':
        provider.updateStudentStatus(student.id, StudentStatus.suspended);
        break;
      case 'verify':
    ...
    showDialog(
      ...
      builder: (context) => EnhancedCreateStudentDialog(
        onStudentCreated: (student) async {
          final result = await context.read<EnhancedStudentProvider>().createStudent(student);
    ...
    showDialog(
      ...
      builder: (context) => EnhancedEditStudentDialog(
        ...
        onStudentUpdated: (student) async {
          final result = await context.read<EnhancedStudentProvider>().updateStudent(student.id, student);
          _showResultMessage(result);
        },
      ),
    );
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              final result = await context.read<EnhancedStudentProvider>().deleteStudent(student.id);
              _showResultMessage(result);
            },
  ...
  void _showBulkDeleteConfirmation() {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Supprimer ${provider.selectedStudents.length} étudiants'),
        content: const Text(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.of(context).pop();
              final result = await provider.deleteSelectedStudents();
              _showResultMessage(result);
            },
            child: const Text('Supprimer'),
            style: TextButton.styleFrom(foregroundColor: Colors.red),
  ...
  void _showExportDialog({bool selectedOnly = false}) {
    showDialog(
      ...
      builder: (context) => EnhancedExportWidget(
        ...
        onExport: (format, filters) async {
          final result = selectedOnly
              ? await context.read<EnhancedStudentProvider>().exportSelectedStudents(format: format)
              : await context.read<EnhancedStudentProvider>().exportStudents(format: format);
          _showResultMessage(result);
          Navigator.of(context).pop();
        },
      ),
  ...
  void _showBulkPromotionDialog() {
    // Show bulk promotion dialog
  }

  void _showBulkGraduationDialog() {
    // Show bulk graduation dialog
  }

  void _showBulkActivationDialog() {
    context.read<EnhancedStudentProvider>().activateSelectedStudents();
  }

  void _showBulkDeactivationDialog() {
    context.read<EnhancedStudentProvider>().deactivateSelectedStudents();
  }

  void _showResultMessage(Map<String, dynamic> result) {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
