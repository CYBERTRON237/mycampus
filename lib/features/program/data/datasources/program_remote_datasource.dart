...
class ProgramRemoteDataSource {
  ...
  final String baseUrl = 'http://localhost/mycampus/api/programs';
  ...
  Future<List<ProgramModel>> getPrograms({
    ...
    try {
      ...
      if (degreeLevel != null) queryParams['degree_level'] = degreeLevel.toJson();
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] == true) {
          ...
          throw Exception(jsonResponse['message'] ?? 'Failed to load programs');
        }
      } else {
  ...
  Future<ProgramModel> getProgramById(String id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] == true) {
          return ProgramModel.fromJson(jsonResponse['data']);
        } else {
          throw Exception(jsonResponse['message'] ?? 'Program not found');
        }
      } else {
        throw Exception('HTTP ${response.statusCode}: ${response.body}');
      }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (jsonResponse['success'] == true) {
          ...
          throw Exception(jsonResponse['message'] ?? 'Failed to create program');
        }
      } else {
        throw Exception('HTTP ${response.statusCode}: ${response.body}');
      }
  ...
  Future<ProgramModel> updateProgram(String id, ProgramModel program) async {
    try {
      final headers = await _getHeaders();
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] == true) {
          ...
          throw Exception(jsonResponse['message'] ?? 'Failed to update program');
        }
      } else {
        throw Exception('HTTP ${response.statusCode}: ${response.body}');
      }
  ...
  Future<void> deleteProgram(String id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] != true) {
          throw Exception(jsonResponse['message'] ?? 'Failed to delete program');
        }
      } else {
        throw Exception('HTTP ${response.statusCode}: ${response.body}');
      }
  ...
  Future<void> toggleProgramStatus(String id, ProgramStatus status) async {
    try {
      final headers = await _getHeaders();
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] != true) {
          throw Exception(jsonResponse['message'] ?? 'Failed to toggle program status');
        }
      } else {
        throw Exception('HTTP ${response.statusCode}: ${response.body}');
      }
    } catch (e) {
      throw Exception('Failed to toggle program status: $e');
    }
  }
