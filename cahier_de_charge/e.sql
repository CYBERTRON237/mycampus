jai deja mon projet en local donc code juste en code brut puis je vais copier rine dautre pas de tsx pas de react salut bolt je voudrais que tu ameliore conciderablement un code pour moi le code du dashboard uniquement lui en generant du code brut broullon a copier ce code devra etr etres volumineux plus de 6000 lignes de code uniquement en dart tu va metre du contenu des actions rapides des indication genre des que un users ce connecte lapplication doit lui dire quoi faire lui presenter genre etapes par etape lapplication doit se comporter comme un guide et passer a letape suivant mais a la prochaine connexion il ne doit plus avoir le guide mais les autre information de lapplication tu va mettre beacuoup de fonction de contenu evite les fauces informations frontend unutiles fais des recher affiche notament les informations necessaires a cette page dashboard en cree des contenues pour chaques type dutilisateurs depasse meme les 10000 lignes de code dart sans probleme tout en respectant les modules deja coder stp assure toi que le dashboard utilise normalement la page dashboard recteur widget que jai cree quand le users est un recteur puis redonne moi le code pour que je puisse le copier et tester en local genere du code brut corriger ameliore et augmete conciderabelment en fonction de lapp nai pas de limite ameliore les couleus les conteues surout pour les teachers les etudiants et les invites met des actions rapide partout ameliore les couleurs et les style aussi genre du code brut broullon en dart uniquement voici la base du fichier :import 'dart:async';
import 'dart:io';
...
bool _isRector() {
  if (_user == null) return false;
  return _user!.role.toLowerCase() == 'univ_rector';
}

bool _isViceRector() {
  if (_user == null) return false;
  return _user!.role.toLowerCase() == 'univ_vice_rector';
}

bool _isSecretaryGeneral() {
  if (_user == null) return false;
  return _user!.role.toLowerCase() == 'univ_secretary';
}
...
String _getRoleDisplayName() {
  ...
  switch (role) {
    ...
    case 'univ_rector': return 'Recteur d\'Université';
