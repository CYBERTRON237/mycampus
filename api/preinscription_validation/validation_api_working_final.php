<?php
// Version finale qui fonctionne - désactive les triggers
error_reporting(0);
...
class PreinscriptionValidationAPI {
    ...
    public function validatePreinscription($preinscriptionId, $adminId, $comments = '') {
        try {
            // Désactiver les triggers et contraintes
            $this->pdo->exec("SET @OLD_SQL_MODE = @@SQL_MODE, SQL_MODE = 'NO_AUTO_VALUE_ON_ZERO'");
            ...
            if (!$user) {
                ...
                if (!$user) {
                    throw new Exception("Impossible de créer le compte utilisateur pour: " . $preinscription['email']);
                }
            }
            ...
            // 4. Mettre à jour préinscription
    ...
    private function createInviteUser($preinscription) {
        try {
            ...
            // Déterminer l'institution par défaut (première institution disponible)
