<?php
// Désactiver l'affichage des erreurs pour éviter XDebug
error_reporting(0);
    ...
    public function getPendingPreinscriptions() {
        try {
            $sql = "SELECT p.*, 
                    ...
                    OR (p.status = 'under_review' AND (p.student_id IS NULL OR p.student_id = 0))
            ...
            foreach ($preinscriptions as &$preinscription) {
                ...
                unset($preinscription['password_hash']);
            }
    ...
    public function getValidationStats() {
        try {
            ...
            $sql = "SELECT COUNT(*) as count FROM preinscriptions 
                    WHERE (status = 'pending' OR (status = 'under_review' AND (student_id IS NULL OR student_id = 0)))";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute();
            ...
            $sql = "SELECT COUNT(*) as count FROM preinscriptions p 
                    ...
                    WHERE p.status IN ('pending', 'under_review') AND (p.student_id IS NULL OR p.student_id = 0)";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute();
